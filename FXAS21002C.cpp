...
FXAS21002C::FXAS21002C(byte addr) {
    address = addr;
    gyroODR = GODR_200HZ;  // In hybrid mode, accel/mag data sample rates are
                           // half of this value
    gyroFSR = GFS_500DPS;
}
...
void FXAS21002C::readRegs(byte reg, uint8_t count, byte dest[]) {
    ...
    Wire1.endTransmission(false);  // Send the Tx buffer, but send a restart to
                                   // keep connection alive
    Wire1.requestFrom(address,
                      count);  // Read bytes from slave register address
...
void FXAS21002C::readTempData() { tempData = readReg(FXAS21002C_H_TEMP); }
...
void FXAS21002C::standby() {
    ...
    writeReg(FXAS21002C_H_CTRL_REG1,
             c & ~(0x03));  // Clear bits 0 and 1; standby mode
}
...
void FXAS21002C::ready() {
    ...
    writeReg(FXAS21002C_H_CTRL_REG1,
             c & ~(0x03));  // Clear bits 0 and 1; standby mode
    writeReg(FXAS21002C_H_CTRL_REG1,
             c | 0x01);  // Set bit 0 to 1, ready mode; no data acquisition yet
}
...
void FXAS21002C::active() {
    ...
    writeReg(
        ...
        c | 0x02);  // Set bit 1 to 1, active mode; data acquisition enabled
}
...
void FXAS21002C::init() {
    ...
    // Disable FIFO, route FIFO and rate threshold interrupts to INT2, enable
    // data ready interrupt, route to INT1 Active HIGH, push-pull output driver
    // on interrupts
    writeReg(FXAS21002C_H_CTRL_REG2, 0x0E);

    // Set up rate threshold detection; at max rate threshold = FSR; rate
    // threshold = THS*FSR/128
    writeReg(FXAS21002C_H_RT_CFG,
             0x07);  // enable rate threshold detection on all axes
    writeReg(FXAS21002C_H_RT_THS,
             0x00 | 0x0D);  // unsigned 7-bit THS, set to one-tenth FSR; set
                            // clearing debounce counter
    ...
    // writeReg(CTRL_REG3, readReg(CTRL_REG3) |  (0x02)); // select ACTIVE HIGH,
    // push-pull interrupts writeReg(CTRL_REG4, readReg(CTRL_REG4) & ~(0x1D));
    // // clear bits 0, 3, and 4 writeReg(CTRL_REG4, readReg(CTRL_REG4) |
    // (0x1D));
    // // DRDY, Freefall/Motion, P/L and tap ints enabled writeReg(CTRL_REG5,
    // 0x01);  // DRDY on INT1, P/L and taps on INT2
...
void FXAS21002C::readGyroData() {
    ...
    readRegs(FXAS21002C_H_OUT_X_MSB, 6,
             &rawData[0]);  // Read the six raw data registers into data array

    gyroData.x =
        ((int16_t)(((int16_t)rawData[0]) << 8 | ((int16_t)rawData[1])));
    gyroData.y =
        ((int16_t)(((int16_t)rawData[2]) << 8 | ((int16_t)rawData[3])));
    gyroData.z =
        ((int16_t)(((int16_t)rawData[4]) << 8 | ((int16_t)rawData[5])));
}
...
int FXAS21002C::setRateODR(GyroODR odr) {
    ...
    if (odr < 7) {
        writeReg(FXAS21002C_H_CTRL_REG1,
                 (readReg(FXAS21002C_H_CTRL_REG1) & 0xE3) | odr << 2);
    }
}
...
int FXAS21002C::setLowPassLPF(GyroLPF lpf) {
    ...
    writeReg(FXAS21002C_H_CTRL_REG0,
             (readReg(FXAS21002C_H_CTRL_REG0) & 0x3f) | lpf << 6);
}
...
int FXAS21002C::setRangeFSR(GyroFSR fsr) {
    ...
    if (fsr <= 3) {
        ...
        writeReg(FXAS21002C_H_CTRL_REG0,
                 (readReg(FXAS21002C_H_CTRL_REG0) & 0xFC) | fsr);
...
void FXAS21002C::calibrate(int samples, int hertz) {
    ...
    for (ii = 0; ii < samples; ii++)  // construct count sums for each axis
    {
        ...
        readRegs(FXAS21002C_H_OUT_X_MSB, 6,
                 &rawData[0]);  // Read the FIFO data registers into data array
        temp[0] =
            ((int16_t)(((int16_t)rawData[0]) << 8 | ((int16_t)rawData[1])));
        temp[1] =
            ((int16_t)(((int16_t)rawData[2]) << 8 | ((int16_t)rawData[3])));
    ...
    gBias[0] = (float)gyro_bias[0] *
               getGres();  /// gyrosensitivity; // get average values
    gBias[1] = (float)gyro_bias[1] *
               getGres();  /// gyrosensitivity; // get average values
    gBias[2] = (float)gyro_bias[2] *
               getGres();  /// gyrosensitivity; // get average values
...
void FXAS21002C::reset() {
    writeReg(FXAS21002C_H_CTRL_REG1,
             0x40);  // set reset bit to 1 to assert software reset to zero at
                     // end of boot process
    delay(100);
    while (!(readReg(FXAS21002C_H_INT_SRC_FLAG) &
             0x08)) {  // wait for boot end flag to be set
    }
}

bool FXAS21002C::begin(gyroRange_t rng) { return true; }

bool FXAS21002C::getEvent(sensors_event_t *event) { return true; }

void FXAS21002C::getSensor(sensor_t *sensor) {}

void FXAS21002C::standby(boolean standby) {}
// Private Methods
// //////////////////////////////////////////////////////////////
